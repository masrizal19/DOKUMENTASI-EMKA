...
export default function App() {
  ...
  const fetchPublicData = async () => {
    ...
    try {
      ...
      const [settingsRes, activitiesRes, categoriesRes, photosRes, vmRes] = await Promise.all([
        ...
        fetchVisionMission().catch(() => ({ data: null, error: null })),
      ]);

      let activeSet: Settings | null = null;
      if (settingsRes?.data?.success && settingsRes.data.data) {
        const rawData = settingsRes.data.data;
        ...
        if (Array.isArray(rawData)) {
          ...
          Object.assign(settingsMap, rawData);
        }

        // Parse missions from vision-mission.php or fallback
        let parsedMissions: string[] = [];
        if (vmRes?.data?.success && Array.isArray(vmRes.data.data) && vmRes.data.data.length > 0) {
          const sortedVm = [...vmRes.data.data].sort((a: any, b: any) => Number(a.display_order || 0) - Number(b.display_order || 0));
          parsedMissions = sortedVm.map((item: any) => item.content || item.title || "");
        } else if (settingsMap.vision_content) {
          parsedMissions = settingsMap.vision_content.split("\n").map((s: string) => s.trim()).filter(Boolean);
        ...
        activeSet = {
          ...
          whatsapp_description: settingsMap.whatsapp_description || "Hubungi admin secara langsung melalui WhatsApp.",
          about_title: settingsMap.about_title || "Tentang Kami & Filosofi Sekolah",
          ...
          hero_description: settingsMap.hero_description || "Elevating School Memories into Fine-Art Archives.",
          hero_image: settingsMap.hero_image || "",
          hero_video: settingsMap.hero_video || "",
          hero_source: settingsMap.hero_display_mode === "manual" ? "manual" : "auto",
          hero_activity_id: settingsMap.hero_manual_activity_id ? String(settingsMap.hero_manual_activity_id) : "",
          sections: ((fallbackData.settings as any)?.sections as SectionSetting[]) || [],
          enable_kegiatan_page: settingsMap.homepage_active_activities === "0" || settingsMap.homepage_active_activities === 0 ? false : true,
          enable_foto_terbaru_page: settingsMap.homepage_active_latest_photos === "0" || settingsMap.homepage_active_latest_photos === 0 ? false : true,
          homepage_gallery_limit: settingsMap.homepage_photo_limit !== undefined && settingsMap.homepage_photo_limit !== "" ? Number(settingsMap.homepage_photo_limit) : 6,
          homepage_gallery_photo_ids: selectedPhotoIds,
          slideshow_duration: settingsMap.slideshow_duration !== undefined && settingsMap.slideshow_duration !== "" ? Number(settingsMap.slideshow_duration) : 5,
